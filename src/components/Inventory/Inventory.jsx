...
import { Typography, Button, List, ListItem, IconButton } from "@mui/material";
...
const Inventory = () => {
  ...
  const equipPog = (e) => {
    dispatch({ type: "UPDATE_USER_EQUIPMENT", payload: e.currentTarget.id });
  ...
  const unequipPog = (e) => {
    dispatch({ type: "UPDATE_USER_INVENTORY", payload: e.currentTarget.id });
          ...
          return (
            <>
              <ListItem key={item.item_id}>
                {item.equippable && (
                  <IconButton id={item.item_id} onClick={equipPog}>
                    <AddItem />
                  </IconButton>
                )}
          ...
          return (
            <>
              <ListItem key={item.id}>
                {item.equippable && (
                  <IconButton id={item.id} onClick={unequipPog}>
                    <RemoveItem />
                  </IconButton>
                )}
